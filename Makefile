...
.PHONY: setup train train-all predict table2
...
setup:
	...
	@$(UV) run python scripts/download_mit_weights.py --model_type b3
...
train:
	...
	@$(UV) run python -m scripts.apply_upstream_overlays --only $(MODEL)
	@$(UV) run python -m scripts.run_upstream $(MODEL)

train-all:
	@$(UV) run python -m scripts.train_all_upstream $(TRAIN_ALL_ARGS)
