...
class DATASET(Dataset):
    def __init__(self, images_path, labels_path, masks_path, transform=None):
        super().__init__()
...
if __name__ == "__main__":
    ...
    train_dataset = DATASET(train_x, train_label, train_y, transform=transform)
    valid_dataset = DATASET(valid_x, valid_label, valid_y, transform=None)

    train_loader = DataLoader(dataset=train_dataset, batch_size=batch_size, shuffle=True, num_workers=2)
    valid_loader = DataLoader(dataset=valid_dataset, batch_size=batch_size, shuffle=False, num_workers=2)

    device = torch.device('cuda')
    model = TGAPolypSeg().to(device)
    ...
    for epoch in range(num_epochs):
        ...
        if valid_loss < best_valid_loss:
            data_str = f"Val loss improved from {best_valid_loss:.4f} to {valid_loss:.4f}. Saving: {checkpoint_path}"
            print_and_save(train_log_path, data_str)
